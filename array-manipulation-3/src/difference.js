...
// outside of the second for loop code block but within the first for loop code block , check if the counter is less than 1

// if so, append element at the index of i of the first array to the new array
// outside of if statement assign 0 to counter

// outside of both for loops, run a run a for loop that iterates through the second array by initializing an i variable and assigning 0 to it, checking if i is less than the length of the second array before running through the for loop, and lastly incrememnting i by 1 at the end of each pass of the for loop
// in the first for loop code block run a for loop that iterates through the first array by initializing an j variable and assigning 0 to it, checking if j is less than the length of the first array before running through the for loop, and lastly incrememnting j by 1 at the end of each pass of the for loop

// in the code block of the first for loop , check if the element at index i of the first array is equal to index j of the second array
// if So, increment counter by  1
